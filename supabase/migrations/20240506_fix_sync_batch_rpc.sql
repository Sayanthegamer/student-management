-- Migration: Update sync_student_fee_batch RPC to use description field
-- Description: Fixes the function to match the fees schema which uses description instead of fine/remarks.
...
BEGIN
    ...
    IF array_length(p_replace_fee_student_ids, 1) > 0 THEN
        ...
        AND (jsonb_typeof(p_fees) != 'array' OR id NOT IN (SELECT id FROM jsonb_to_recordset(p_fees) AS x(id UUID)));
    END IF;
