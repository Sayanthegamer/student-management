-- Migration: Update sync_student_fee_batch RPC to persist remarks and itemized_breakdown
-- Description: The fee INSERT previously omitted the `description` (packed remarks/fine) and
--              `itemized_breakdown` columns, causing those fields to be dropped for transportation
--              fees and any other fees written via the bulk-replace path.
...
BEGIN
    ...
    -- 2. Bulk upsert fees — now including description and itemized_breakdown
    IF jsonb_array_length(p_fees) > 0 THEN
        INSERT INTO public.fees (id, student_id, date, amount, type, month, fine, description, itemized_breakdown)
        SELECT id, student_id, date, amount, type, month, fine, description, itemized_breakdown
...
-- Revoke and grant execute permissions (unchanged)
