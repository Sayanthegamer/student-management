-- Migration: Extend sync_student_fee_batch RPC with full student column set
-- Description: Drops and recreates the function to include all student columns,
-- preventing silent field drops during bulk upserts (guardian_name, dob, phone,
-- address, admission_fee, concession_amount, enrollment_type, admission_number, etc.)
...
BEGIN
    -- Stage 1: Bulk upsert students with full column set to prevent silent field drops
    ...
    -- Stage 2: Bulk upsert fees using the description field (not fine/remarks)
    ...
    -- Stage 3: Delete orphaned fees for targeted students not present in the incoming fees batch
