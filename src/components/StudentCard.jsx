...
import { Edit2, IndianRupee, Trash2, Zap, ChevronDown, ChevronUp } from 'lucide-react';
    ...
    <div className={`card-base p-3 transition-all duration-200 group hover:bg-[var(--bg-card-hover)] spotlight-card relative overflow-hidden ${isSelected ? 'ring-2 ring-[var(--accent-primary)] bg-[var(--accent-primary)]/5' : ''}`}>
      ...
      {isExpanded && (
        <div className="mb-3 space-y-2 border-t border-[var(--border-subtle)] pt-2 mt-2">
            ...
            <div className="flex justify-between text-xs">
                <span className="text-[var(--text-muted)]">RTE Status:</span>
                <span className="text-[var(--text-primary)] font-medium">{student.isRTE ? 'Yes' : 'No'}</span>
            </div>
            {student.transportArea && (
                <div className="flex justify-between text-xs">
                    <span className="text-[var(--text-muted)]">Transport Area:</span>
                    <span className="text-[var(--text-primary)] font-medium">{student.transportArea}</span>
                </div>
            )}
        </div>
      )}
