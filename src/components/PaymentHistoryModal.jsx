...
const PaymentHistoryModal = ({ student, onClose }) => {
    ...
    return createPortal(
        ...
        >
            <div className={`bg-[#0a0a0a] rounded-none shadow-none w-full max-w-2xl max-h-[calc(100vh-3rem)] md:max-h-[90vh] mx-auto my-4 md:my-auto flex flex-col overflow-hidden border border-[#CCFF00] ${isClosing ? 'scale-out' : 'scale-in'}`}>
                ...
                <div className="bg-[#CCFF00] px-6 py-6 md:py-8 text-black relative flex-shrink-0 border-b border-[#CCFF00]">
                    ...
                    <button
                        onClick={handleClose}
                        className="absolute top-4 right-4 md:top-6 md:right-6 p-3 min-h-[48px] min-w-[48px] bg-transparent border border-black hover:bg-black hover:text-[#CCFF00] text-black rounded-none transition-colors z-20 flex items-center justify-center"
                    >
                ...
                <div className="overflow-y-auto p-4 md:p-8 flex-1 bg-[#0a0a0a]">
                    {sortedHistory.length === 0 ? (
                        <div className="text-center py-20 bg-[#050505] rounded-none border border-white/20">
                        ...
                        <div className="overflow-hidden rounded-none border border-white/20 shadow-none bg-[#050505]">
                            <table className="hidden md:table w-full text-left border-collapse">
                                <thead className="bg-[#050505] text-white/50 text-[10px] uppercase tracking-widest font-black border-b border-white/20">
                                    <tr>
                                        <th className="px-5 py-4 border-b border-white/20">Date</th>
                                        <th className="px-5 py-4 border-b border-white/20">Period</th>
                                        <th className="px-5 py-4 border-b border-white/20 text-right">Base</th>
                                        <th className="px-5 py-4 border-b border-white/20 text-right">Fine</th>
                                        <th className="px-5 py-4 border-b border-white/20 text-right">Total</th>
                                    </tr>
                                </thead>
                                    ...
                                    {sortedHistory.map((payment) => (
                                        <tr key={payment.id} className="hover:bg-white/5 transition-colors">
                                            ...
                                            <td className="px-5 py-5">
                                                <span className="bg-[#CCFF00] text-black px-2 py-1 rounded-none text-[9px] font-black uppercase tracking-widest border border-[#CCFF00]">
                                                    {payment.month}
                                                </span>
                                            </td>
                                ...
                                <tfoot className="bg-[#0a0a0a] text-white border-t border-white/20 font-black">
                                    <tr>
                            ...
                            <div className="md:hidden flex flex-col divide-y divide-white/10">
                                    ...
                                    <div key={payment.id} className="p-5 bg-transparent">
                                        <div className="flex justify-between items-start mb-2">
                                            <div>
                                                ...
                                                <div className="mt-3">
                                                    <span className="inline-block bg-[#CCFF00] text-black px-2 py-1 rounded-none text-[9px] font-black uppercase tracking-widest border border-[#CCFF00]">
                                                        {payment.month}
                                                    </span>
                                                </div>
                                            </div>
                                ...
                                <div className="bg-[#0a0a0a] p-5 text-white border-t border-white/20">
